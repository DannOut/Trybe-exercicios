...
const allNameStudentsArray = [];

for (let index = 0; index < estudantes.length; index += 1 ) {
  if(estudantes[index].turno === 'Manhã') {
    allNameStudentsArray.push(`${estudantes[index].nome} ${estudantes[index].sobrenome}`)
  }
}
console.log(allNameStudentsArray);


const allNameStudentsFilterMap = estudantes.filter((estudante) => (
  estudante.turno === 'Manhã')).map((estudante) => `${estudante.nome} ${estudante.sobrenome}`);

console.log(allNameStudentsFilterMap);
...
const reportStatusFindMap = (name, students) => {
...
console.log(reportStatusFindMap('Natalia', estudantes));